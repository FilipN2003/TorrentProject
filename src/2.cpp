...
int main(int argc, char* argv[])
{
    ...
    for(int i=1;i<argc;++i){
        ...
        lt::error_code ec;
        lt::bdecode_node node = lt::bdecode(span_buffer, ec);
        if(ec){
            std::cerr << "bdecode failed for " << path << ": " << ec.message() << "\n";
            continue;
        }

        lt::torrent_info t_info(node,ec);
        if(ec){
            std::cerr << "Failed to create torrent_info for " << path << ": " << ec.message() << "\n";
            continue;
        }

        std::cout << "Dodajem torrent: " << t_info.name() << "\n";
    ...
    if(handles.empty()){
        std::cerr << "Nijedan torrent nije uspesno dodat.\n";
        return 1;
    }
    ...
    for(;;){
        ...
        for(auto& handle : handles){
            ...
            std::cout << "[" << status.name << "] "
                      << (status.progress*100) << "% | "
                      << "DL: " << (status.download_rate / 1000) << "kB/s "
                      << "UL: " << (status.upload_rate / 1000) << "kB/s "
